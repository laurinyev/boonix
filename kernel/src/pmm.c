...
#define PURE(n) (uintptr_t)(((uintptr_t)n - lowest_free_page)/PAGE_SIZE)
...
void pmm_init(struct limine_memmap_response* memmap) {
    ...
    for(size_t i = 0; i < memmap->entry_count; i++){
        ...
        if (e->type != LIMINE_MEMMAP_USABLE && e->base >= usable_mem_start && e->base + e->length <= usable_mem_start + total_range) {
            mark_region(PURE(e->base),e->length);
        }
    }
...
void* pmm_alloc() {
    for(uintptr_t c = lowest_free_page; PURE(c) <= bitmap_size; c+=PAGE_SIZE){
        uintptr_t c_pure = PURE(c);
        uintptr_t offset = c_pure / 8;
        uint8_t   bit    = c_pure % 8;
        uint8_t   byte   = bitmap_ptr[offset];
        kprintf("c = 0x%llx; c_pure = 0x%x; byte = 0x%x; bit = 0x%x\n",c,c_pure,byte,bit);
        if (byte == 0xFF) {
            c+= PAGE_SIZE * ((8-bit)-1);
            continue;
        }
